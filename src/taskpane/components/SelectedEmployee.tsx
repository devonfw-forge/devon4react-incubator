const getSelectedEmployeeData = async (context: Excel.RequestContext) => {
    ...
    const range = activeSheet.context.workbook.getSelectedRange().load(["address", "values", "rowIndex", "formulas"]); // Get the selected cell location, value and index of its row
    await context.sync();
